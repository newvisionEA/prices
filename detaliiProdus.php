...
require 'db.php';
		...
		$query = "SELECT p.pack_id packid, p.id pid, p.name pname, b.name bname, qty_um, um, c.name cname, c.img cimg, s.city city, s.address adr, pri.value pval, p.month_stock pms, 
		(
SELECT MIN( value ) 
FROM price
WHERE product_id =".$id."
...
AND p.id=".$id." order by pval";
	    ...
	    while($row = mysql_fetch_array($result)) {
	        ...
	        if ($pval > $pmin) {
	          ...
	          $output .= '<td>platesti cu '.
	          number_format(floor($temp), 0, '.', '').
	          '</B><SUP>'.substr(number_format($temp - floor($temp), 2, '', ''), 1).'</SUP>'.
	          ' Lei mai mult pe luna</td>';
	        } else {
